...
<sf:form method="POST" action="${saveUrl}" modelAttribute="function">
	<div class="form-header">
		<h3 class="form-header-title"><s:message code="~de.unibamberg.minf.dme.form.function.edit" /> <small><span class="glyphicon glyphicon-info-sign help-sign" onclick="functionEditor.showHelp(); return false;" aria-hidden="true"></span></small></h3>	
		<sf:hidden path="id" />
	...
	<div class="form-content row" style="padding-bottom: 0px;">
		<div class="col-md-7" style="border-right: 1px solid #E5E5E5;">
			<div class="legend"><strong>1</strong> <s:message code="~de.unibamberg.minf.dme.form.function.legend.edit_function" /></div>
			<div class="form-group row">
				...
				<div class="col-sm-4">
					<label class="control-label" for="base_method"><s:message code="~de.unibamberg.minf.dme.model.function.language_version" />:</label>
					<div>
			...
			<div class="form-group">
				<label class="control-label"><s:message code="~de.unibamberg.minf.dme.model.function.state" />:</label>
				<span class="function_state"><span class="glyphicon glyphicon-ok-sign" aria-hidden="true"></span> <s:message code="~de.unibamberg.minf.common.link.ok" /> </span>
				<button class="btn btn-info btn-sm pull-right" onclick="functionEditor.processFunction(); return false;"><span class="glyphicon glyphicon-cog" aria-hidden="true"></span> <s:message code="~de.unibamberg.minf.common.link.validate" /></button>
			</div>
			<div class="form-group">
				<label class="control-label"><s:message code="~de.unibamberg.minf.dme.model.function.grammar" />:</label>
				<span>${grammar.name}</span> (<span class="grammar_state">
					<c:choose>
						<c:when test="${grammar.passthrough}">
							<span class="glyphicon glyphicon-forward" aria-hidden="true"></span> <s:message code="~de.unibamberg.minf.dme.model.grammar.passthrough" />
						</c:when>
						<c:when test="${grammar.error}">
							<span class="glyphicon glyphicon-exclamation-sign glyphicon-color-danger" aria-hidden="true"></span> <s:message code="~de.unibamberg.minf.common.link.error" />
						</c:when>
						<c:otherwise>
							<span class="glyphicon glyphicon-ok-sign" aria-hidden="true"></span> <s:message code="~de.unibamberg.minf.common.link.ok" />
						</c:otherwise>
					</c:choose>
				</span>)
			</div>
			<div class="form-group">
				<div class="row">
					<div class="col-sm-8">
						<label class="control-label" for="function_function"><s:message code="~de.unibamberg.minf.dme.model.function.transformation_function" />:</label>
						<div>
					...
					<div class="col-sm-4">
						<label class="control-label"><s:message code="~de.unibamberg.minf.dme.notification.transformation.available_inputs" />:</label>
						<div class='available-input-container'>
							<c:if test="${availablePassthroughGrammars!=null && fn:length(availablePassthroughGrammars)>0}">
								<em><s:message code="~de.unibamberg.minf.dme.notification.transformation.available_passthrough_grammars" /></em>
								<ul>
							...
							<c:if test="${availableRules!=null && fn:length(availableRules)>0}">
								<em><s:message code="~de.unibamberg.minf.dme.notification.transformation.available_parser_rules" /></em>
								<ul>
		...
		<div class="col-md-5" style="border-left: 1px solid #E5E5E5; margin-left: -1px;">
			...
			<div>
				<div class="sample-input-container">
					<c:forEach var="sampleInput" items="${sampleInputMap}">
						...
						<div class="sample-input form-group">
							<label class="control-label"><s:message code="~de.unibamberg.minf.dme.editor.sample.input" /> <em>(${sampleInput.key.name}):</em></label>
							<div>
				...
				<div class="clearfix">
					<c:choose>
						<c:when test="${grammar.error}">
							<button class="btn btn-warning disabled btn-sm pull-right"><span class="glyphicon glyphicon-exclamation-sign" aria-hidden="true"></span> <s:message code="~de.unibamberg.minf.dme.editor.sample.process_input" /></button>
						</c:when>
						<c:otherwise>
							<button class="btn btn-info btn-sm pull-right" onclick="functionEditor.performTransformation(); return false;"><span class="glyphicon glyphicon-play" aria-hidden="true"></span> <s:message code="~de.unibamberg.minf.dme.editor.sample.process_input" /></button>
						</c:otherwise>
					</c:choose>
				</div>
			</div>
